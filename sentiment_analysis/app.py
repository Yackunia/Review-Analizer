...
def generate_company_details(name: str) -> Dict[str, Any]:
    prompt = (
        ...
        'Если организация имеет несколько точек, то анализируй всю компанию целиком, а не конкретные места.'
        ...
        '    "reviewsCount": 0,\n'
        '    "sources": список из ссылок источников отзывов, доменов ссылок для каждого источника и краткий анализ отзывов с этого источника (всего не больше пяти троек).'
        '        Домены источников не должны повторяться.'
        '        (возвращай источники в формате одного списка: [ссылка_0, домен_0, анализ_0, ссылка_1, домен_1, анализ_1, ...])\n'
